...
//	}
	...
	public static class Modifier {
		...
		public Modifier(final int color, final float glow, final int glowColor) {
			super();
			this._color = color;
			this._glow = glow;
		...
		public UpdateFlags modify(final Drawing d){return null;}
	}
	
	public SVGDrawable(final InputStream is, final Modifier modifier) {
		super();
	...
	public SVGDrawable(final Drawing d, final Modifier modifier) {
		super();
		//this._agr = new AndGraphicsRenderer();//TODO move out
		this._modifier=modifier;
		this._drawing=d;
	...
	public SVGDrawable(final SVGDrawable root, final Modifier modifier) {
		super();
		//this._agr = root._agr;
		this._parent=root;
		this._modifier=modifier;
...
//				);
	...
	public void setIntrinsicBounds(final Rect defaultBounds) {
	...
	public void setIntrinsicSquare(final float defaultBounds) {
		if (this._intrinsicBounds==null) {this._intrinsicBounds=new Rect();}
	...
	public void setClipping(final Rect clipping) {
	...
	public void draw(final Canvas canvas) {
		final DrawingElement de = getElement();
		//if (true) return;// test 
		if (de!=null) {
			UpdateFlags flags = UpdateFlags.ALL;
			//p.start();
			//_drawing.update(true, _agr, flags);
			final Rect bounds = getBoundsRect();
			final int dWidth = bounds.width();//-padding.left-padding.right // - getPaddingLeft() - getPaddingRight();
			final int dHeight = bounds.height();//-padding.top-padding.bottom //getMeasuredHeight() - getPaddingTop() - getPaddingBottom();
			...
			yscaling=xscaling=1;
			final ViewPortData vpd = ViewPortData.getFragmentViewPort(de);//ViewPortData.getFullDrawing(d);
			_tl.set(0, 0);
			...
			//if (_debug) {
			getPadding(_useRect);
            final Rect padding =_useRect;
			final String file = (String)_drawing.getNameSpaced("co.uk.sentinel", "file");
...
//			p.dumpAccum("svg"+file);
	...
	private void loadDrawing(final InputStream is) {
		try {
			final InputSource isc = new InputSource(is);
			final SVGParser svgp = new SVGParser();
